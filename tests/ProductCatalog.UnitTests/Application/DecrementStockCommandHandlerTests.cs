...
public class DecrementStockCommandHandlerTests
{
    ...
    {
        _handler = new DecrementStockCommandHandler(_repository.Object);
    }

    [Fact]
    public async Task Handle_SufficientStock_ReturnsProductAfterDecrement()
    {
        _repository.Setup(r => r.TryDecrementStockAsync(100001, 5, It.IsAny<CancellationToken>())).ReturnsAsync(true);
        ...
        _repository.Verify(r => r.TryDecrementStockAsync(100001, 5, It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task Handle_InsufficientStock_ReturnsInsufficientStockFailure()
    {
        _repository.Setup(r => r.TryDecrementStockAsync(100001, 10, It.IsAny<CancellationToken>())).ReturnsAsync(false);
        _repository.Setup(r => r.GetByIdAsync(100001, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Product.Create(100001, "Mouse", null, 10m, 3));
    ...
    {
        _repository.Setup(r => r.TryDecrementStockAsync(999999, 5, It.IsAny<CancellationToken>())).ReturnsAsync(false);
